...
@Component({
  selector: 'app-coworkingform',
  templateUrl: './coworkingform.component.html',
  styleUrls: ['./coworkingform.component.scss']
})
export class CoworkingformComponent implements OnInit {
  ...
  constructor(public login: LoginService, public coworking: CoworkingService, public dictionary: DiccionarioService,
    private log: LogsService, private ngSelectConfig: NgSelectConfig, private route: ActivatedRoute ) {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(
      res => {
        object = JSON.parse(res['params']['form']);
        this.formCow = object;
      }
    )
    
    this.count = object?object:0;
