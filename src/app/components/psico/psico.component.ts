...
@Component({
  ...
  styleUrls: ['./psico.component.scss'],
  styles: [
    `
    :host {
      >>> .popover {
...
export class PsicoComponent implements OnInit {
  ...
  constructor(private login: LoginService, private booking: BookingService, private psico: PsicoService, private route: ActivatedRoute, private log:LogsService) {
  ...
  saveData() {
    ...
    if (todasRespondidas) {
      ...
      Swal.fire({
        ...
        text: `Por favor diligencia todos los campos para continuar.`,
        type: 'error',
        confirmButtonText: 'Aceptar'
      });
      return;
